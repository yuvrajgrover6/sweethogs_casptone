...
export class ReadmissionService {
  ...
  /**
   * Analyzes diabetic patient data and returns only the confidence score
   */
  async predictReadmissionScore(
    ...
    try {
      ...
      const analysis = this.analyzePatientRisk(patientData);

      // Calculate overall confidence score (0-100)
      const confidence_score = Math.round(analysis.overall_risk * 100);
      ...
      throw new BaseErrorException({
        ...
        logInfo: { error, patientId: patientData.patient_nbr },
        code: 500,
      });
    }
  }
  ...
  /**
   * Analyzes diabetic patient data and predicts readmission risk
   */
  async predictReadmission(
    ...
    try {
      ...
      const risk_level = this.determineRiskLevel(confidence_score);

      // Identify risk factors
      const risk_factors = this.identifyRiskFactors(patientData, analysis);
